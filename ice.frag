...
vec3 getStars (vec3 position, float t) {
    vec3 starposition = vec3(position * 9000. + t);
...
void main() {
    ...
    // flicker lightness
    float t2 = abs(sin(mod(time *.8, 1000.)));
    float lightness = t2 * .25;
    color.rgb += lightness;
    ...
    // test gradation via y value
    // float blueamount = clamp(exp(vPosition.y), .65, .8);
    // color.rgb = mix(blue, color.rgb, blueamount);
    // vec3 white = vec3(.6);
    // float whiteamount = clamp(exp(-vPosition.y), .4, .9);
    // color.rgb = mix(white, color.rgb, whiteamount);

    // contrast again
    contrast = 1.9; //1. + dist;
    color.rgb = ((color.rgb - 0.5) * max(contrast, 0.)) + .5;

    // wow it's water almost
    // color -= 1.-dist;
    // color.a = 1.;
