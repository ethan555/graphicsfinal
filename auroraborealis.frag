...
void main( void ) {
    ...
    vec2 position = ( world_position.xz ) / (resolution.y) + (mouse_y * 2. - 1.);
    ...
    if (mouse_x < .5) {
        color.xyz = mix(color1.xyz, color2.xyz, mouse_x*2.) + stars;
    } else {
        color.xyz = mix(color2.xyz, color3.xyz, (mouse_x - .5)*2.) + stars;
    }
