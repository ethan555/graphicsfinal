...
void main( void ) {
    ...
    vec2 mousepos = (mouse * resolution) * scale;
    ...
    float mousedist = distance(gl_FragCoord.xy*scale, mousepos);
    vec3 randposition = vec3(position * 5., t);
    float value = 2.*pow(6.*pow(.5 + gradient_octaves(randposition, 2)*.5, 6.), 2.);
    float starvalue = 0.;
    vec3 stars = vec3(0.);

    //value *= 1./pow(dist/(100.*scale),100.);
    //value *= ((100.*scale) / (dist - 100.));
    value *= pow((100.*scale)/max(70.*scale, dist), 10.);
    if (mousedist < 100.*scale) {
	value *= pow(mousedist/(100.*scale),10.);
	//value *= ((100.*scale) / (dist - 100.));
        //value *= pow((100.*scale)/dist, 10.);
    } else {
	vec3 starposition = vec3(position * 1000., t2);//1.);
